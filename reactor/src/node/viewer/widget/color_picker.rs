...
use egui::epaint::ecolor::{Color32, Hsva, HsvaGamma, Rgba};
use egui::epaint::{Mesh, Rect, Shape, Stroke, StrokeKind, Vec2, pos2, vec2};
use egui::{
    Area, Context, DragValue, Frame, Id, Key, Order, Painter, Pos2, Response, Sense, Ui, UiKind, Widget, WidgetInfo,
    WidgetType, epaint, lerp, remap_clamp,
};

use super::fixed_cache::FixedCache;
...
pub fn color_button(ui: &mut Ui, color: Color32, open: bool) -> Response {
    ...
    if ui.is_rect_visible(rect) {
        ...
        ui.painter().rect_stroke(
            rect,
            corner_radius,
            (stroke_width, visuals.bg_fill), /* Using fill for stroke is intentional, because default style has no
                                              * border */
            StrokeKind::Inside,
        );
    }
...
///
/// e.g.: `|x_value, y_value| HsvaGamma { h: 1.0, s: x_value, v: y_value, a: 1.0 }.into()` displays the colors as
/// follows:
...
fn srgba_edit_ui(ui: &mut Ui, [r, g, b, a]: &mut [u8; 4], alpha: Alpha) -> bool {
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button("📋").on_hover_text("Click to copy color values").clicked() {
            if alpha == Alpha::Opaque {
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button("📋").on_hover_text("Click to copy color values").clicked() {
            if alpha == Alpha::Opaque {
                ...
                ui.ctx().copy_text(format!("{r:.03}, {g:.03}, {b:.03}, {a:.03}"));
            }
        }
...
pub fn color_edit_button_hsva(ui: &mut Ui, hsva: &mut Hsva, alpha: Alpha) -> Response {
    ...
    if button_response.clicked() {
        ...
        if ui.memory(|mem| mem.is_popup_open(popup_id)) {
            let pos = ui.input(|i| i.pointer.latest_pos()).unwrap_or(button_response.rect.max);
            ui.memory_mut(|mem| mem.data.insert_temp(popup_id, pos));
        } else {
            ui.memory_mut(|mem| mem.data.remove::<Pos2>(popup_id));
        }
    }
    ...
    if ui.memory(|mem| mem.is_popup_open(popup_id)) {
        // let pos = button_response.rect.max;

        let area_response = Area::new(popup_id)
            .kind(UiKind::Picker)
            .order(Order::Foreground)
            .fixed_pos(ui.memory(|mem| mem.data.get_temp(popup_id).unwrap_or(button_response.rect.max)))
            .show(ui.ctx(), |ui| {
